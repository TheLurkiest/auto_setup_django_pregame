...
# ------------------- START OF .CSV INPUT INTO 7 PYTHON MODULES UPON USER PROMPTS -------------------
	...
	echo '	path('admin/', admin.site.urls),' >> u2_alone_config
	echo "	re_path('api/(?P<version>(v1|v2))/', include('api.urls'))" >> u2_alone_config
	...
	for file in dir[1-99]; do
		...
		do
			...
			then
				...
				echo "	def get(self, request, version, format=None):" >> v2_file
				echo "		${m1,,} = ${m1}.objects.all()" >> v2_file
				echo "		serializer = ${m1}Serializer(${m1,,}, many=True)" >> v2_file
				echo "		return Response(serializer.data)" >> v2_file

				# url_pat2="View.as_view(), name=\"user-all\""

				echo "urlpatterns = [" >> u2_main_file
				echo "	('${m1,,}/', ${m1}View.as_view(), name=\"${m1,,}-all\"" >> u2_main_file
				echo "]" >> u2_main_file
				echo "" >> u2_main_file
				...
				echo "	class Meta:" >> s2_file
				echo "		model = ${m1}" >> s2_file
